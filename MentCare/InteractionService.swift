...
struct InteractionService {
    // Tehlikeli ilaç çiftleri ve uyarı mesajları
    ...
    // Verilen ilaç listesinde bir çakışma olup olmadığını kontrol eder
    static func checkInteractions(for drugs: [String]) -> String? {
        ...
        for (dangerousPair, warning) in knownInteractions {
            // Eğer tehlikeli çiftin her iki ilacı da doktorun yazdığı listede varsa
